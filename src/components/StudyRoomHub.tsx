...
export const StudyRoomHub: React.FC<StudyRoomHubProps> = ({ currentProfile, onOpenRoom }) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">Phòng họp học nhóm</h2>
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Camera, chia sẻ màn hình, cùng xem YouTube và thu nhỏ để tiếp tục dùng web.</p>
          </div>
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
        {loading ? (
          ...
          <div className="sm:col-span-2 rounded-2xl border border-dashed border-indigo-300 dark:border-indigo-700 p-6 text-center text-sm text-slate-600 dark:text-slate-300">
            Chưa có phòng đang mở. Hãy tạo phòng đầu tiên để học, họp hoặc cùng xem tài liệu.
          </div>
        ) : rooms.map((room) => (
      ...
      {showCreate && (
        <div className="fixed inset-0 z-[205] bg-slate-950/70 p-4 flex items-center justify-center" onClick={(event) => { if (event.target === event.currentTarget) setShowCreate(false); }}>
          <form onSubmit={createRoom} className="w-full max-w-md rounded-[2rem] bg-white dark:bg-slate-900 p-6 shadow-2xl">
            <div className="flex items-center justify-between"><h2 className="text-xl font-black text-slate-900 dark:text-white">Tạo phòng họp</h2><button type="button" onClick={() => setShowCreate(false)} className="p-2 text-slate-400"><X className="w-5 h-5" /></button></div>
            ...
            <button type="submit" disabled={creating} className="mt-6 w-full min-h-12 rounded-xl bg-indigo-600 text-white font-black disabled:opacity-60 inline-flex items-center justify-center gap-2">{creating && <Loader2 className="w-4 h-4 animate-spin" />} Tạo và vào phòng họp</button>
          </form>
        </div>
      )}
